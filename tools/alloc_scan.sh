#!/usr/bin/env bash
#
# alloc_scan.sh — whether the lookup path ever allocates, as a command
# that either passes or names the function that broke it.
#
...
# paints.  `ugrapheme.break_step` is what a terminal emulator calls
# once per codepoint.  `uclass`'s predicates are a lexer's inner loop.
# The `@tier(embedded)` annotations on all three say they reach no
# allocator.  That annotation is checked against a list of admitted
# stdlib entries rather than against the emitted code, so this reads
# the emitted code.
#
# `novo build` writes the module's LLVM to `_novo/<name>.ll` before it
# invokes clang, so the IR is there to read.  `--opt=0` keeps the
# functions separate.  At the default optimisation level the whole
# probe inlines into `novo_main`, there is nothing left to attribute an
...
# The lookup path, which is the width rule, the break machine, the
# predicates, and the table search all three are written on.
hot = re.compile(r'^novo_user_(uwidth_|uclass_|ugrapheme_|utbl_)')

# The half that does not build for a device, excluded by name rather
# than by the scan quietly not looking at it.  Each of these carries
# `@tier(app)` in the source and allocates deliberately.  A cursor is a
# boxed struct holding the caller's string, `clusters` and `words` build
# a list, `category` and its neighbours reach the tables behind a
# `UniData`, and `utbl`'s packed-blob searches read a `[u8]` the host
# handed over.
