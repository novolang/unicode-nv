...
WHAT IT EMITS, and why a table is a string literal rather than a list.
A `const` list is inlined at every use site (docs/language/guide.md,
...
and both `str.len` and `str.byte_at_or` are admitted at
`@tier(embedded)` — so the same table serves the host and a Cortex-M
with no allocator.  Every table below is therefore a byte string, read
through `utbl`.
...
TWO SHAPES OF TABLE:
  ...
  Used where a property has a value everywhere: width, the break
  classes, the predicate bits, the general category, the combining
  class, the quick-check bits.

  A KEYED table is a sorted run of `(key, value)` entries searched for
  an exact match.  Used where most codepoints have no entry at all:
  the case mappings, the decompositions, the compositions.
...
class Module:
    ...
    def __init__(self, name, headline, note):
        ...
        self.lines = [
            ...
            % UNICODE_VERSION,
            "// Character Database.  Do not edit by hand: regenerate it with",
            "//",
...
# ── the tier-one tables: width, breaks, predicates ───────────────────
...
def gen_core(ucd, rows):
    ...
    textwrap-nv's `WrapWidth` holds.  Together they are what the device
    claim covers.
    """
    m = Module(
        "udbcore",
        ...
        """
TIER ONE.  `uwidth`, `ugrapheme` and `uclass`'s predicates read these
and take no data handle, so they cost a program nothing to reach and
run at `@tier(embedded)`.  The three tier-two tables are in
`udbcat`, `udbcase` and `udbnorm`, behind a `UniData`.
""",
    )
...
in bits 0..2 — the declaration order of `uwidth.UniEastAsian` — and
bit 3 set when the codepoint occupies no cell at all.
""",
    )
...
11..12 — the declaration order of `ugrapheme`'s own tables in each
case.  One table rather than four because the four properties change
at nearly the same codepoints, so four tables would cost four searches
...
def gen_cat(rows):
    m = Module(
        ...
        """
TIER TWO, reached through `udata.UniData`.  `uclass.category` is the
one function that needs the whole classification rather than the
handful of bits `udbcore.PRED` carries, and it is the reason that
function takes a data handle where the predicates beside it do not.
""",
    )
    cat = {}
    for cp, f in rows.items():
...
def gen_case(ucd, rows):
    m = Module(
        ...
        """
TIER TWO, reached through `udata.UniData`.
...
codepoint and is what a caller walking codepoints wants; a full
...
each, with U+0000 meaning "no mapping" — which is never a mapping
...
FULLPOOL — lowercase, uppercase, titlecase, folded — with a length of
zero meaning the simple mapping is the whole answer.
""",
    )
    m.table(
...
def gen_norm(ucd, rows):
    m = Module(
        ...
        """
TIER TWO, reached through `udata.UniData`, and the largest tables in
the package.
...
not the recursive form: `unorm.decompose_char` recurses, and storing
the closure would repeat every shared tail.
""",
    )
    m.table(
    ...
    m.table(
        "COMP", bytes(data), 9,
        """
The primary composites, keyed on the PAIR: the starter, the combining
codepoint and what they compose to, sorted so one search over nine-byte
entries finds a pair.  A composite that Full_Composition_Exclusion
names is absent, which is the whole reason this is a table of its own
rather than DECOMP read backwards.
""",
    )
...
NFD, NFC, NFKD, NFKC: 0 is Yes, 1 is No, 2 is Maybe.  This is the
table `unorm.quick_check` reads, and reading it is the whole of why
that function can answer without allocating.
""",
    )
    return m
